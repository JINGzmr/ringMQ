...
//是整个消息队列集群的「大脑」，负责集群元数据管理、Broker 负载均衡、Partition 状态管控、主从切换、消费订阅管理等核心能力
import (
...
func (z *ZkServer) ProGetBroker(info Info_in) Info_out {
	...
	for BrokerName, BrokerHostPort := range Brokers {
		...
		if resp == nil || !resp.Ret {
			if resp != nil {
				logger.DEBUG(logger.DError, "%v\n", resp.Err)
				return Info_out{Err: errors.New(resp.Err)}
			}
			return Info_out{Err: errors.New("prepare accept returned nil response")}
		}
		...
		if PartitionNode.Option == -2 { //未设置状态
			...
			if resp == nil || !resp.Ret {
				...
				return Info_out{Err: errors.New("prepare state returned nil response")}
			}
		}
	}
